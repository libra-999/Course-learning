    ...
    <div
        ...
        <div class="pt-5 px-3">
            <div class="flex items-center gap-3">
                <img class=" size-10 object-cover rounded-full border-2 border-gray-300"
                    :src="`${props.bookData.volumeInfo.imageLinks?.thumbnail}`" alt="empty image">
                <div class="border-l-2 border-gray-300 pl-3 flex flex-col gap-1">
                    <h3 class="text-base font-semibold text-title-50  leading-5 line-clamp-1 text-left">
                        {{ props.bookData.volumeInfo.authors?.[0] }}
                    </h3>
                    <p class="text-sm  opacity-50 leading-5 text-left">
                        {{ props.bookData.volumeInfo.maturityRating }}</p>
                    <small
        ...
        <!-- Thumbnail -->
        <div class="w-full h-40.75 mx-auto overflow-hidden px-4 mt-5 cursor-pointer  rounded-lg" @click="getBook(bookData.id)">
            <!-- <a :href="props.bookData.selfLink" target="_blank" rel="noreferrer"> -->
                <img :src="`${props.bookData.volumeInfo.imageLinks?.smallThumbnail}`"
                    class="w-full h-full object-cover rounded-lg" alt="empty image">
            <!-- </a> -->
        </div>
        <!-- Title book -->
        <div class="p-5 mb-2">
        ...
        <div class="absolute right-1 top-0 etag">
            <el-tag class="text-[10px]! opacity-60 text-emerald-600!">{{ props.bookData.etag }}</el-tag>
        </div>
        <!-- Rating book -->
        ...
        <small class=" absolute right-2.5 bottom-3 text-gray-300">{{ props.bookData.volumeInfo.pageCount }} pages</small>
    </div>
    <!-- Detail Book-->
