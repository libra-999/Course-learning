...
function filterAsyncRouter(asyncRouteMap: any[], type: boolean = false) {
   return asyncRouteMap.filter (route => route.menuType !== 2) // no filter button permission
       .map (route => {
          const children = route.children || []
          
          route.path = getNormalPath (route.path).toLocaleLowerCase ()
          ...
          const childRoute = filterAsyncRouter (children, type)
